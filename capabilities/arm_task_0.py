#!/usr/bin/env python3
META = {"name": "Arm Task 0", "icon": "0️⃣", "description": "Send arm task_id=0"}
...
def main():
    if OFFLINE:
        print("OFFLINE: arm task_id=0 simulated"); return
    rclpy.init()
    class N(Node):
        def __init__(self):
    ...
    node = N()
    time.sleep(0.5)
    print("Sending arm task_id=0...")
    msg = Request()
    msg.header.identity.api_id = 7106
    msg.parameter = json.dumps({"data": 0})
